  ...
  "relative w-full rounded-lg px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        success:
          "text-emerald-500 bg-emerald-100 [&>svg]:text-current *:data-[slot=alert-description]:text-emerald-500",
        warning:
          "text-orange-800 bg-orange-100 [&>svg]:text-current *:data-[slot=alert-description]:text-orange-800",
        destructive:
